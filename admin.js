/* ============================================================
   admin.js
   Logic for the private studio / admin panel (admin.html).
   This is now its own page — it keeps its own `products` /
   `orders` / `adminSelected` state and sets up its own live
   Firebase listeners (via storage.js) instead of sharing them
   with user.js.
   Load order in admin.html: config.js -> storage.js -> api.js -> admin.js
   ============================================================ */
...
// Wires the admin panel's own controls (tabs, new product, back link).
...
/* ============================================================
   Password gate — protects direct visits to admin.html.
   If the user already unlocked it via the 5-click gate on
   user.html in this browser tab (see user.js), sessionStorage
   skips the prompt. Otherwise it asks again, right here.
   ============================================================ */
function showAdminGate(){
  $('#adminGate').style.display = 'flex';
  $('#adminShellWrap').style.display = 'none';
  setTimeout(() => $('#gatePwInput')?.focus(), 100);
}
function unlockAdmin(){
  sessionStorage.setItem('afsAdminUnlocked', '1');
  $('#adminGate').style.display = 'none';
  $('#adminShellWrap').style.display = 'block';
  renderAdmin();
  renderAdminOrders();
}
function checkGatePassword(){
  ...
  if(val === ADMIN_PASSWORD){
    ...
    $('#gatePwError').textContent = 'Galat password. Dobara try karo.';
    $('#gatePwInput').value = '';
    $('#gatePwInput').focus();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#gatePwSubmit')?.addEventListener('click', checkGatePassword);
  $('#gatePwInput')?.addEventListener('keydown', e => { if(e.key==='Enter') checkGatePassword(); });
  $('#backToStore')?.addEventListener('click', () => { location.href = 'user.html'; });

  if(sessionStorage.getItem('afsAdminUnlocked') === '1'){
    $('#adminGate').style.display = 'none';
    $('#adminShellWrap').style.display = 'block';
  } else {
    showAdminGate();
  ...
  // Products + orders load live from Firebase, same database the
  // storefront reads/writes — edits here show up on every device.
  watchProducts(list => {
